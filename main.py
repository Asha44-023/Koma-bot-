...
def get_exchanges():
    ...
    if len(api_key)>10 and len(secret)>10:
        print(f"Keys found len {len(api_key)} -> private ON")
        ...
        if ex_private:
            try:
                for p in ex_private.fetch_positions([SYMBOL]):
                    ...
                    reason=None
                    if side=='short' and c>o and body_ratio>=BODY_EXIT_RATIO: reason=f"BODY BREAK {body_ratio:.2f}"
                    if side=='long' and c<o and body_ratio>=BODY_EXIT_RATIO: reason=f"BODY BREAK {body_ratio:.2f}"
                    if side=='short' and c > last_flip*(1+FLIP_BREAK_PCT): reason=f"FLIP BROKE {last_flip:.5f}->{c:.5f}"
                    if side=='long' and c < last_flip*(1-FLIP_BREAK_PCT): reason=f"FLIP BROKE {last_flip:.5f}->{c:.5f}"
                    if up_r>=3.0 and low_r>=3.0: reason=f"BOTH WICKS {up_r:.1f}x/{low_r:.1f}x"
                    if reason:
                        ...
                        send_telegram(f"⚠️ *KOMA AUTO EXIT {side.upper()}* {reason} at {price:.5f}")
                        return
            except Exception as e: print(f"KOMA exit err {e}")

        print(f"KOMA {price:.5f} body {body_ratio:.2f} Up {up_r:.1f}x Low {low_r:.1f}x")
        if is_pick and low_r>=1.2 and can_send_koma(price):
            COOLDOWN_KOMA.setdefault("flips",{})[SYMBOL]=FLOOR; save_koma()
            send_telegram(f"🟢 *KOMA BUY* Low {low_r:.1f}x at {price:.5f} Floor {FLOOR:.5f}")
        if is_pick and up_r>=1.2 and can_send_koma(price):
            COOLDOWN_KOMA.setdefault("flips",{})[SYMBOL]=CEIL; save_koma()
            send_telegram(f"🔴 *KOMA SELL* High {up_r:.1f}x at {price:.5f} Ceil {CEIL:.5f}")

    except Exception as e: print(f"KOMA scan err {e}"); import traceback; traceback.print_exc()
        ...
        if ex_private:
            try:
                for p in ex_private.fetch_positions([sym]):
                    ...
                    if side=='short' and c>last_flip*(1+FLIP_BREAK_PCT): reason=f"FLIP BROKE {last_flip:.5f}->{c:.5f}"
                    if side=='long' and c<last_flip*(1-FLIP_BREAK_PCT): reason=f"FLIP BROKE {last_flip:.5f}->{c:.5f}"
                    if up_r>=3.0 and low_r>=3.0: reason=f"BOTH WICKS VOL EXIT"
                    if reason:
                        ...
                        send_telegram(f"⚠️ *{sym} AUTO EXIT {side.upper()}* {reason} at {price:.5f}")
                        return
            except Exception as e: print(f"Exit {sym} err {e}")

        print(f"{sym} {price:.5f} body {body_ratio:.2f} Up {up_r:.1f}x Low {low_r:.1f}x")
        if is_pick and low_r>=1.2 and can_send_other(sym, price):
            COOLDOWN_OTHER.setdefault("flips",{})[sym]=FLOOR; save_other()
            send_telegram(f"🟢 *{sym} BUY* Low wick {low_r:.1f}x at {price:.5f}")
        if is_pick and up_r>=1.2 and can_send_other(sym, price):
            COOLDOWN_OTHER.setdefault("flips",{})[sym]=CEIL; save_other()
            send_telegram(f"🔴 *{sym} SELL* High wick {up_r:.1f}x at {price:.5f}")
...
def main():
    ...
    print(f"\n=== BOTH 100% SCAN {session} UTC {hour_utc} PICK={is_pick} ===")
