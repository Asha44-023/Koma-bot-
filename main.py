...
def detect_out_in(o,h,l,c, side):
    ...
    if side=="BUY":
        lvl = min(l[-6:-1])
        swept = c[-2] < lvl*0.997
        back_inside = c[-1] > lvl and c[-1] > o[-1]
        dist = (lvl - min(l[-2],l[-3]))/lvl*100 if swept else 0
        return swept and back_inside and 0.15 <= dist <= 0.7
    else:
        lvl = max(h[-6:-1])
        swept = c[-2] > lvl*1.003
        back_inside = c[-1] < lvl and c[-1] < o[-1]
...
def check_exits():
    ...
    for s in list(ACTIVE.keys()):
        ...
        key=pos.get("side_key", s)
        if is_buy:
            if cur>=tp2:
                tg(f"🎯 TP2 HIT {s} ERL")
                COOLDOWN["signals"][key]={"t":now,"dir":is_buy,"result":"TP2"}
