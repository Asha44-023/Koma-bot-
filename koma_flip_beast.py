...
# === KOMA BEAST V3 - 10% PUMP + 4H TIME STOP ===
PUMP_TRIGGER = 10.0   # was 20
DUMP_TRIGGER = -10.0
TP_PCT = 8.0
SL_PCT = 5.0
TIME_STOP_HOURS = 4
...
def get_position_entry_time(position_info):
    try:
        # MEXC gives createTime or openTime in info
...
def scalp_plan(ex, free_bal, send_telegram, can_send_func):
    try:
        # 1. Check existing position + TIME STOP
        ...
        # 2. No position -> Look for 10% move
        try:
            # 1h candles for % change
            ohlcv = ex.fetch_ohlcv(SYMBOL, '1h', limit=5)
            df = pd.DataFrame(ohlcv, columns=['t','o','h','l','c','v'])
            if len(df) < 2:
                return "NO DATA"
            
            # Change from 2 candles ago (1h change)
