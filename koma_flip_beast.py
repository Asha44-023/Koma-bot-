...
# === KOMA ULTRA BEAST V5 - MAX TRADES 5% + 1.0x VOL ===
PUMP_TRIGGER = 5.0   # ULTRA - was 10
DUMP_TRIGGER = -5.0  # ULTRA - was -10
TP_PCT = 5.0         # FAST CLOSE - was 8
SL_PCT = 4.0         # TIGHT - was 5
TIME_STOP_HOURS = 2  # FAST ROTATION - was 4
VOL_MULT = 1.0       # NO FILTER - was 1.3
    ...
    try:
                ...
                if abs(contracts) > 0:
                    ...
                    if open_ts:
                        ...
                        if hours_open >= TIME_STOP_HOURS:
                            try:
                                ...
                                send_msg(send_telegram, f"⏰ *ULTRA TIME STOP {TIME_STOP_HOURS}H* {side.upper()} {hours_open:.1f}h PNL {pnl_pct:.2f}%")
                    ...
                    if pnl_pct >= TP_PCT:
                        try:
                            close_side = "sell" if "long" in side else "buy"
                            ex.create_market_order(SYMBOL, close_side, abs(contracts), params={"reduceOnly": True})
                            send_msg(send_telegram, f"💰 *ULTRA TP {TP_PCT}%* {side.upper()} +{pnl_pct:.2f}%")
                    ...
                    if pnl_pct <= -SL_PCT:
                        try:
                            close_side = "sell" if "long" in side else "buy"
                            ex.create_market_order(SYMBOL, close_side, abs(contracts), params={"reduceOnly": True})
                            send_msg(send_telegram, f"✂️ *ULTRA SL {SL_PCT}%* {side.upper()} {pnl_pct:.2f}%")
        ...
        try:
            ...
            if vol_now < vol_avg * VOL_MULT and abs(change_1h) < PUMP_TRIGGER:
                return f"WAIT Vol {vol_ratio:.1f}x < {VOL_MULT}x change {change_1h:.2f}%"
            
            if change_1h >= PUMP_TRIGGER:
                if not can_send_func(SYMBOL, f"PUMP{change_1h:.0f}", 20):
                ...
                send_msg(send_telegram, f"🔴 *ULTRA SHORT {change_1h:.2f}%* Vol {vol_ratio:.1f}x Price {price:.6f} Size ${notional} TP {TP_PCT}% SL {SL_PCT}%")
                return f"SHORT PUMP {change_1h:.2f}%"
            
            if change_1h <= DUMP_TRIGGER:
                if not can_send_func(SYMBOL, f"DUMP{change_1h:.0f}", 20):
                ...
                send_msg(send_telegram, f"🟢 *ULTRA LONG {change_1h:.2f}%* Vol {vol_ratio:.1f}x Price {price:.6f} Size ${notional} TP {TP_PCT}% SL {SL_PCT}%")
                return f"LONG DUMP {change_1h:.2f}%"
            
            return f"WAIT change {change_1h:.2f}% Vol {vol_ratio:.1f}x need {PUMP_TRIGGER}%"
