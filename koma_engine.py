...
def check_other_coin(sym,df5m,df15m,df1h):
    ...
    if vol_r>=1.5: score+=2; reasons.append(f"VOL INCREASE x{vol_r:.1f} REAL BUY"); buy+=2
    elif vol_r>=1.2: score+=1; reasons.append(f"VOL UP x{vol_r:.1f}"); buy+=1
    elif vol_r<=0.6: score-=1; reasons.append(f"VOL DECREASE x{vol_r:.1f} SELL / fake"); sell+=1
    ...
    if len(df5m)>=2:
        ...
        if up_r>1.5 and prev_up>1.5: score-=2; reasons.append(f"DOUBLE WICK FAKE top 2x - NO TRADE")
    ...
    if vol_r>=2.5 and body < (h-df5m['low'].iloc[-1])*0.3: score+=2; reasons.append(f"WHALE MANIPULATION vol {vol_r:.1f}x small body"); buy+=1
    score=max(0,min(10,score))
...
if __name__=="__main__":
    ...
    session,hour_utc,is_pick=get_killzone_pick()
    print(f"=== SCAN START {session} UTC {hour_utc
