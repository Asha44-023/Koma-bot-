    ...
    try:
        if TELEGRAM_TOKEN and TELEGRAM_CHAT:
            requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage",
                         json={"chat_id": TELEGRAM_CHAT, "text": msg, "parse_mode": "Markdown"}, timeout=15)
    except: pass
    print(msg)
...
def can_send_killzone(sym):
    ...
    if LAST_ALERT.get("date") != today:
        LAST_ALERT["count_today"] = 0; LAST_ALERT["date"] = today
    if LAST_ALERT.get("count_today",0) >= MAX_SIGNALS_PER_DAY: return False, session
    k = f"{sym}_SIGNAL"
    if time.time() - LAST_ALERT.get(k,0) < SIGNAL_COOLDOWN_MIN*60: return False, session
    return True, session
...
def check_koma(df5m, df15m, df1h):
    ...
    if low_r>2.5: score+=3; reasons.append(f"LOW GRAB {low_r:.1f}x FLOOR {FLOOR:.5f} LONG 9/10"); buy+=3
    if up_r>1.8 and price<CEIL: score+=2; reasons.append(f"UP REJECT {up_r:.1f}x CEIL {CEIL:.5f}"); buy+=2
    if abs(price-FLIP)/FLIP<0.004: score+=2; reasons.append(f"AT FLIP {FLIP:.5f}"); buy+=1
    r5=rsi(df5m).iloc[-1]
    if r5<30: score+=2; reasons.append(f"RSI OS {r5:.0f}"); buy+=2
    elif 30<r5<65: score+=1; reasons.append(f"RSI OK {r5:.0f}")
    vol_r=df5m['volume'].iloc[-1]/(df5m['volume'].rolling(20).mean().iloc[-1] or 1)
    if vol_r>=1.2: score+=1; reasons.append(f"VOL UP x{vol_r:.1f} REAL")
...
# ===== OTHER COINS LOGIC - WHALE + LIQ + VOLUME =====
def detect_whale(df):
    o=df['open'].iloc[-1]; c=df['close'].iloc[-1]; h=df['high'].iloc[-1]; l=df['low'].iloc[-1]
    body=abs(c-o) or 0.0001
    upper=h-max(o,c); lower=min(o,c)-l
    if lower>body*2.5 and upper<body*0.5: return "BULL_LIQ_GRAB", f"BULL LIQ GRAB {lower/body:.1f}x LONG"
    if upper>body*2.5 and lower<body*0.5: return "BEAR_LIQ_GRAB", f"BEAR LIQ GRAB {upper/body:.1f}x SHORT"
...
def check_other_coin(sym, df5m, df15m, df1h):
    ...
    if whale_type=="BULL_LIQ_GRAB": score+=3; reasons.append(f"🐋 {whale_msg} 9/10"); buy+=3
    if whale_type=="BEAR_LIQ_GRAB": score+=3; reasons.append(f"🐋 {whale_msg} 9/10"); sell+=3

    # 2. Volume logic - you asked: volume increase = buy, decrease = sell
    ...
    if len(df5m)>=2:
        ...
        if up_r>1.5 and prev_up>1.5:
            score-=2; reasons.append(f"DOUBLE WICK FAKE top 2x - NO TRADE")

    # 4. Momentum 2UP/2DOWN
    c0,c1,c2=df5m['close'].iloc[-3:].values
    mom5=((c0-c1)/c1*100) if c1>0 else 0
    if c0>c1>c2 and mom5>0.1: score+=1; reasons.append(f"5m 2UP {mom5:.2f}%"); buy+=1
    if c0<c1<c2 and mom5<-0.1: score+=1; reasons.append(f"5m 2DOWN {mom5:.2f}%"); sell+=1

    # 5. RSI + Location
    r5=rsi(df5m).iloc[-1]
    low_24=df1h['low'].tail(24).min(); high_24=df1h['high'].tail(24).max()
    loc=(price-low_24)/(high_24-low_24)*100 if high_24>low_24 else 50
    if loc<20 and r5<40: score+=2; reasons.append(f"BOTTOM {loc:.0f}% RSI {r5:.0f} LONG"); buy+=2
    elif loc>80 and r5>60: score+=2; reasons.append(f"TOP {loc:.0f}% RSI {r5:.0f} SHORT"); sell+=2

    # 6. Whale manipulation - big volume + small body = manipulation
    if vol_r>=2.5 and body < (h-df5m['low'].iloc[-1])*0.3:
        score+=2; reasons.append(f"WHALE MANIPULATION vol {vol_r:.1f}x small body"); buy+=1

    score=max(0,min(10,score))
    if buy>=3 and score>=7: decision="BUY NOW"
    elif sell>=3 and score>=7: decision="SELL NOW"
...
if __name__ == "__main__":
    ...
    print(f"Watchlist: KOMA (wick) + {OTHER_SYMBOLS} (whale/liq/vol)")
    print(f"Pick hours: {PICK_HOURS} UTC = Asian 03-06, London 11-14, NY 16-19 Kalimoni")
    while True:
        try:
            for sym in KOMA_SYMBOLS + OTHER_SYMBOLS:
                try:
                    ...
                    if "KOMA" in sym:
                        ...
                        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] {sym} {session} {kalimoni}:00 Kalimoni | Price {price:.5f} Wick {FLOOR:.5f}-{CEIL:.5f} | {decision} {score}/10")
